package com.lautarorisso.notification_service.domain.service;
...
/**
 * Domain service for resolving notification targets from incident events.
 * <p>
...
public class NotificationRoutingService {
    ...
    public Set<UUID> resolveTargets(Map<String, Object> event) {
        ...
        return switch (eventType) {
            case "INCIDENT_ASSIGNED" -> {
                String assigneeStr = (String) event.get("assigneeId");
                if (assigneeStr == null) {
                    yield Collections.emptySet();
                }
                UUID assignee = UUID.fromString(assigneeStr);
                String changedByStr = (String) event.get("changedBy");
                if (changedByStr != null) {
                    UUID changedBy = UUID.fromString(changedByStr);
                    if (assignee.equals(changedBy)) {
                        yield Collections.emptySet();
                    }
                }
                yield Set.of(assignee);
            }
            case "INCIDENT_STATUS_CHANGED", "INCIDENT_PRIORITY_CHANGED" -> {
                String assigneeStr = (String) event.get("assigneeId");
                if (assigneeStr == null) {
                    yield Collections.emptySet();
                }
                yield Set.of(UUID.fromString(assigneeStr));
            }
    ...
    public NotificationType resolveNotificationType(String eventType) {
        ...
        return switch (eventType) {
            ...
            case "INCIDENT_PRIORITY_CHANGED" -> NotificationType.INCIDENT_PRIORITY_CHANGED;
            default -> null;
        };
    }
    ...
    public String buildTitle(NotificationType type) {
        return switch (type) {
            ...
            case INCIDENT_PRIORITY_CHANGED -> "Incident priority changed";
            case INCIDENT_COMMENT_ADDED -> "New comment on incident";
        };
    }
}
